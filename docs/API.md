...
请求头：`X-Demo-User-Id`。返回当前用户今日剩余抽卡次数，供 H5 首次加载和切换身份时展示：

```json
{"draws_remaining_today": 3}
```
...
公开接口，用于分享落地页，返回赠送者 ID、昵称、角色、卡片内容、状态与过期时间。
赠送者 ID 和角色仅用于本地 H5 过滤不符合等级规则的演示身份；接口不返回双方额度或领取人信息。
